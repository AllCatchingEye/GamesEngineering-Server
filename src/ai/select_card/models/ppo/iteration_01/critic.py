...
from ai.nn_helper import NUM_ROUNDS, NUM_STACK_CARDS, get_one_hot_encoding_index_from_card, one_hot_encode_cards
from ai.select_card.models.model_interface import ModelInterface

from ai.select_card.models.ppo.iteration_01.shared import INPUT_SIZE, NUM_ENCODED_STACK, Encoding
from ai.select_card.models.shared.nn_state import as_params_file, load_and_apply_parameters_to_model, persist_model_parameters
...
class CriticNN(nn.Module):
    def __init__(self, layers: list[int]):
        ...
        # Critic just outputs one value: the q-value used for advantage function
        all_layers = [INPUT_SIZE] + layers + [1]
...
class PpoCriticIter01(ModelInterface):
    __logger = logging.getLogger("PpoCriticIter01")

    def __init__(self, layers: list[int]):
        self.layers = layers
        self.model = CriticNN(layers)
    ...
    def decode_output(self, output: torch.Tensor) -> float:
    ...
    def get_model_params_path(self, game_type: Gametype) -> str:
        ...
        params_path = ["params", "critic_%s"%"x".join([str(it) for it in self.layers])]
