...
from ai.select_card.models.ppo.iteration_01.shared import INPUT_SIZE, NUM_ENCODED_STACK, OUTPUT_SIZE, Encoding
...
class PpoActorIter01(ModelInterface):
    __logger = logging.getLogger("PpoActorIter01")

    def __init__(self, layers: list[int]):
        self.layers = layers
        self.model = ActorNN(layers)
    ...
    def get_model_params_path(self, game_type: Gametype) -> str:
        ...
        params_path = ["params", "actor_%s"%"x".join([str(it) for it in self.layers])]
